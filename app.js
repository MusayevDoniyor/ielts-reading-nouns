...
const EXERCISES = [
  {
    ...
    icon: "📚",
    label: "Study Cards",
    desc: "Learn with flip cards",
  },
  {
    id: "matching-synonyms",
    icon: "🔗",
    label: "Matching Synonyms",
    desc: "25s per question",
  },
  {
    id: "matching-words",
    icon: "🔤",
  ...
  },
  {
    id: "sentence-making",
    icon: "✍️",
    label: "Sentence Making",
    desc: "15 words, 2 min each",
  },
];
...
function startExercise(unitNum, type) {
  switch (type) {
    case "study-cards":
      startStudyCards(unitNum);
      break;
    case "matching-synonyms":
      startQuiz(unitNum, CONFIGS.matchingSynonyms);
      break;
    case "matching-words":
      startQuiz(unitNum, CONFIGS.matchingWords);
      break;
    case "matching-definitions":
      startQuiz(unitNum, CONFIGS.matchingDefinitions);
      break;
    case "gap-filling":
      startQuiz(unitNum, CONFIGS.gapFilling);
      break;
    case "sentence-making":
      startSentenceMaking(unitNum);
      break;
  }
}
...
dom.btnScPrev.addEventListener("click", () => {
  if (scIndex > 0) {
    scIndex--;
    showStudyCard(scIndex);
  }
});

dom.btnScNext.addEventListener("click", () => {
  if (scIndex < scWords.length - 1) {
    scIndex++;
    showStudyCard(scIndex);
  }
});
...
function handleQuizAnswer(optIndex, config) {
  ...
  feedbackEl.textContent = isCorrect
    ? "Correct!"
    : `Wrong. The answer was: ${q.options.find((o) => o.isCorrect).text}`;
...
function showResultsScreen(data) {
  ...
  const pct =
    data.total > 0 ? ((data.correct / data.total) * 100).toFixed(1) : "0.0";
